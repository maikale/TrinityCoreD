...
void WorldSession::HandleSetupWarbandGroups(WorldPackets::Character::SetupWarbandGroups& setupWarbandGroups)
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_WARBAND_GROUPS_BY_ACCOUNT);
    ...
    for (auto const& group : setupWarbandGroups.Groups)
    {
        ...
        for (uint32 memberIndex = 0; memberIndex < group.Members.size(); ++memberIndex)
        {
            auto const& member = group.Members[memberIndex];

            stmt = CharacterDatabase.GetPreparedStatement(
                CHAR_INS_WARBAND_GROUP_MEMBER);

            stmt->setUInt64(0, group.GroupID);
            stmt->setUInt8(1, memberIndex);
